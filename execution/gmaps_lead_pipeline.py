...
def send_slack_notification(results: dict):
    ...
    message = {
        "text": f"📍 *Google Maps Scrape Complete*",
        "blocks": [
            {
                "type": "header",
                "text": {
                    "type": "plain_text",
                    "text": "📍 Maps Scrape Complete",
                    "emoji": True
                }
            },
            {
                ...
                "fields": [
                    ...
                    {"type": "mrkdwn", "text": f"*Found:*\n{results['businesses_found']} businesses"}
                ]
            },
            {
                "type": "section",
                "fields": [
                    {"type": "mrkdwn", "text": f"*Enriched:*\n{results['leads_enriched']} leads"},
                    {"type": "mrkdwn", "text": f"*Added to Sheet:*\n{results['leads_added']} new leads"}
                ]
            },
            {
                "type": "section",
                "text": {
                    "type": "mrkdwn",
                    "text": f"<{results['sheet_url']}|View Google Sheet>" if results['sheet_url'] else "No sheet URL"
                }
            }
        ]
    }
...
Examples:
  ...
  python3 execution/gmaps_lead_pipeline.py --search "lawyers" --limit 10 --sheet-url "https://docs.google.com/spreadsheets/d/..."
    ...
    parser.add_argument("--sheet-url", default=os.getenv("LEADS_SHEET_URL"), help="Existing Google Sheet URL to append to (defaults to LEADS_SHEET_URL env var)")
