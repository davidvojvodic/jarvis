...
def enrich_sheet(sheet_url):
    ...
    email_col = None
    email_header_name = None
    headers = worksheet.row_values(1)
    
    # Priority: owner_email (best for people), then emails (plural), then email
    target_headers = ["owner_email", "emails", "email"]
    
    for target in target_headers:
        for col_idx, header in enumerate(headers):
            if header.lower() == target:
                email_col = col_idx + 1
                email_header_name = header
                break
        if email_col:
            break
    
    if not email_col:
        print(f"Error: Could not find any email column (checked: {', '.join(target_headers)}) in sheet")
        return None
        
    print(f"Using email column: '{email_header_name}'")
    ...
    for idx, record in enumerate(records):
        ...
        # Check if email is missing using the actual header name found
        email = str(record.get(email_header_name, "")).strip()
        if email and email != "None":
            continue  # Email already exists
        
        # Extract required fields
        # Try standard fields first, then fall back to Gmaps Pipeline fields
        first_name = record.get("first_name", "").strip()
        last_name = record.get("last_name", "").strip()
        full_name = record.get("full_name", "").strip() or record.get("owner_name", "").strip()
        company_domain = record.get("company_domain", "").strip() or record.get("website", "").strip()
        company_name = record.get("company_name", "").strip() or record.get("business_name", "").strip()
...
def main():
    ...
    parser.add_argument("--sheet-url", default=os.getenv("LEADS_SHEET_URL"), help="Google Sheet URL to enrich (defaults to LEADS_SHEET_URL env var)")

    args = parser.parse_args()

    if not args.sheet_url:
        print("Error: No sheet URL provided. Set LEADS_SHEET_URL env var or pass --sheet-url")
        sys.exit(1)
